...
A Python post-processing wrapper for [PrusaSlicer](https://www.prusa3d.com/en/page/prusaslicer_en/) that automatically injects MakerBot Replicator 2-specific Start G-code and converts sliced G-code to Sailfish X3G binary format.
...
The wrapper logic in `gpx_wrapper.py` is currently specifically for the **MakerBot Replicator 2** startup sequence and coordinates.
...
2. **Ensure GPX binary is present** in the same directory as `gpx_wrapper.py`
3. **Configure PrusaSlicer post-processing** (see Usage section below)

### Command Line

```bash
python gpx_wrapper.py "C:\path\to\file.gcode.pp"
```
...
3. Under **G-code substitutions** or **Post-processing scripts**, add:
   ...
   ```bash
   "<PATH_TO_PYTHON>\python.exe" "<PATH_TO_GPX_WRAPPER>\gpx_wrapper.py";
   ```

   Replace `<PATH_TO_PYTHON>` with your Python installation directory and `<PATH_TO_GPX_WRAPPER>` with the directory where `gpx_wrapper.py` is installed.

   PrusaSlicer automatically passes the generated file path to the script.
...
```sh
.
├── gpx_wrapper.py           # Python post-processing script
...
- Ensure `gpx.exe` is in the same directory as `gpx_wrapper.py`
...
- **Python 3.10+** (for `gpx_wrapper.py`)
